const View = {
    ...
    Metadata: {
        ...
        setVal(data, resource){
            ...
            $(resource).find(".data-date").val(metadata.data.length != 0 ? metadata.data[0].language : "")
            $(resource).find(".data-language").val(metadata.data.length != 0 ? metadata.data[0].data_date : "")
        },  
    },
    SideModal: {
        Create: {
